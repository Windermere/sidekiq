...
module Sidekiq1
  ...
  module TestingClient
    def raw_push(payloads)
      if Sidekiq1::Testing.fake?
        payloads.each do |job|
          job = Sidekiq1.load_json(Sidekiq1.dump_json(job))
          job.merge!('enqueued_at' => Time.now.to_f) unless job['at']
          Queues.push(job['queue'], job['class'], job)
        end
        true
      elsif Sidekiq1::Testing.inline?
        payloads.each do |job|
          klass = Sidekiq1::Testing.constantize(job['class'])
          job['id'] ||= SecureRandom.hex(12)
          job_hash = Sidekiq1.load_json(Sidekiq1.dump_json(job))
  ...
  Sidekiq1::Client.prepend TestingClient
  ...
  module Queues
    ...
    #   assert_equal :something, Sidekiq1::Queues["default"].first['args'][0]
    #
  ...
  module Worker
    ##
    # The Sidekiq1 testing infrastructure overrides perform_async
    ...
    #
    #   assert_equal 0, Sidekiq1::Extensions::DelayedMailer.jobs.size
    #   assert_equal 0, Sidekiq1::Extensions::DelayedModel.jobs.size
    #
    #   MyMailer.delay.send_welcome_email('foo@example.com')
    #   MyModel.delay.do_something_hard
    #
    #   assert_equal 1, Sidekiq1::Extensions::DelayedMailer.jobs.size
    #   assert_equal 1, Sidekiq1::Extensions::DelayedModel.jobs.size
    #
    #   Sidekiq1::Worker.clear_all # or .drain_all
    #
    #   assert_equal 0, Sidekiq1::Extensions::DelayedMailer.jobs.size
    #   assert_equal 0, Sidekiq1::Extensions::DelayedModel.jobs.size
    #
    ...
    module ClassMethods
      ...
      def process_job(job)
        ...
        Sidekiq1::Testing.server_middleware.invoke(worker, job, job['queue']) do
      ...
      def drain_all
        while jobs.any?
          ...
          worker_classes.each do |worker_class|
            Sidekiq1::Testing.constantize(worker_class).drain
          end
        end
      end
    end
...
if defined?(::Rails) && Rails.respond_to?(:env) && !Rails.env.test?
  puts("**************************************************")
  puts("⛔️ WARNING: Sidekiq1 testing API enabled, but this is not the test environment.  Your jobs will not go to Redis.")
  puts("**************************************************")
end
