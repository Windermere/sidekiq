...
class TestCli < Sidekiq1::Test
  describe 'CLI#parse' do

    before do
      @cli = Sidekiq1::CLI.new
      @opts = Sidekiq1.options.dup
    end

    after do
      Sidekiq1.options = @opts
    end
    ...
    it 'changes concurrency' do
      ...
      assert_equal 60, Sidekiq1.options[:concurrency]
    end
    ...
    it 'changes concurrency with ENV' do
      begin
        ...
        assert_equal 60, Sidekiq1.options[:concurrency]
        @cli.parse(['sidekiq', '-r', './test/fake_env.rb'])
        assert_equal 9, Sidekiq1.options[:concurrency]
      ensure
    ...
    it 'changes queues' do
      ...
      assert_equal ['foo'], Sidekiq1.options[:queues]
    end

    it 'accepts a process index' do
      @cli.parse(['sidekiq', '-i', '7', '-r', './test/fake_env.rb'])
      assert_equal 7, Sidekiq1.options[:index]
    end
    ...
    it 'sets verbose' do
      ...
      assert_equal Logger::DEBUG, Sidekiq1.logger.level
      # If we leave the logger at DEBUG it'll add a lot of noise to the test output
      Sidekiq1.options.delete(:verbose)
    ...
    describe 'with logfile' do
      ...
      it 'sets the logfile path' do
        ...
        assert_equal @tmp_log_path, Sidekiq1.options[:logfile]
      end

      it 'creates and writes to a logfile' do
        @cli.parse(['sidekiq', '-L', @tmp_log_path, '-r', './test/fake_env.rb'])

        Sidekiq1.logger.info('test message')
    ...
    describe 'with pidfile' do
      ...
      it 'sets pidfile path' do
        assert_equal @tmp_path, Sidekiq1.options[:pidfile]
      end
    ...
    describe 'with config file' do
      ...
      it 'parses as expected' do
        assert_equal './test/config.yml', Sidekiq1.options[:config_file]
        refute Sidekiq1.options[:verbose]
        assert_equal './test/fake_env.rb', Sidekiq1.options[:require]
        assert_nil Sidekiq1.options[:environment]
        assert_equal 50, Sidekiq1.options[:concurrency]
        assert_equal '/tmp/sidekiq-config-test.pid', Sidekiq1.options[:pidfile]
        assert_equal '/tmp/sidekiq.log', Sidekiq1.options[:logfile]
        assert_equal 2, Sidekiq1.options[:queues].count { |q| q == 'very_often' }
        assert_equal 1, Sidekiq1.options[:queues].count { |q| q == 'seldom' }
      end
    end
    ...
    describe 'with config file using string keys' do
      ...
      it 'parses as expected' do
        assert_equal './test/string_config.yml', Sidekiq1.options[:config_file]
        refute Sidekiq1.options[:verbose]
        assert_equal './test/fake_env.rb', Sidekiq1.options[:require]
        assert_nil Sidekiq1.options[:environment]
        assert_equal 50, Sidekiq1.options[:concurrency]
    ...
    describe 'with env based config file' do
      ...
      it 'parses as expected' do
        assert_equal './test/env_based_config.yml', Sidekiq1.options[:config_file]
        refute Sidekiq1.options[:verbose]
        assert_equal './test/fake_env.rb', Sidekiq1.options[:require]
        assert_equal 'staging', Sidekiq1.options[:environment]
        assert_equal 5, Sidekiq1.options[:concurrency]
        assert_equal '/tmp/sidekiq-config-test.pid', Sidekiq1.options[:pidfile]
        assert_equal '/tmp/sidekiq.log', Sidekiq1.options[:logfile]
        assert_equal 2, Sidekiq1.options[:queues].count { |q| q == 'very_often' }
        assert_equal 1, Sidekiq1.options[:queues].count { |q| q == 'seldom' }
    ...
    describe 'with an empty config file' do
      ...
      it 'takes a path' do
        ...
        assert_equal @tmp_path, Sidekiq1.options[:config_file]
      end

      it 'should have an identical options hash, except for config_file' do
        @cli.parse(['sidekiq'])
        old_options = Sidekiq1.options.clone

        @cli.parse(['sidekiq', '-C', @tmp_path])
        new_options = Sidekiq1.options.clone
    ...
    describe 'with config file and flags' do
      ...
      it 'gives the expected options' do
        assert_equal 100, Sidekiq1.options[:concurrency]
        assert_equal @tmp_lib_path, Sidekiq1.options[:require]
        assert_equal 'snoop', Sidekiq1.options[:environment]
        assert_equal @tmp_path, Sidekiq1.options[:pidfile]
        assert_equal 7, Sidekiq1.options[:queues].count { |q| q == 'often' }
        assert_equal 3, Sidekiq1.options[:queues].count { |q| q == 'seldom' }
      end
    end

    describe 'Sidekiq1::CLI#parse_queues' do
    ...
    describe 'Sidekiq1::CLI#parse_queue' do
      describe 'when weight is present' do
  ...
  describe 'misc' do
    ...
    describe 'handles TSTP and USR2' do
      ...
      it 'shuts down the worker' do
        count = 0
        Sidekiq1.options[:lifecycle_events][:quiet] = [proc {
          count += 1
        }]
        @cli.launcher = Sidekiq1::Launcher.new(Sidekiq1.options)
      ...
      it 'reopens logs' do
        ...
        Sidekiq1::Logging.stub(:reopen_logs, mock) do
    ...
    it 'can fire events' do
      count = 0
      Sidekiq1.options[:lifecycle_events][:startup] = [proc {
        count += 1
      }]
      cli = Sidekiq1::CLI.new
