...
class TestScheduled < Sidekiq2::Test
  ...
  describe 'poller' do
    before do
      ...
      @retry = Sidekiq2::RetrySet.new
      @scheduled = Sidekiq2::ScheduledSet.new
      @poller = Sidekiq2::Scheduled::Poller.new
    end
    ...
    it 'executes client middleware' do
      Sidekiq2.client_middleware.add Stopper
      begin
        ...
        assert_equal 0, Sidekiq2::Queue.new("queue_1").size
        assert_equal 1, Sidekiq2::Queue.new("queue_2").size
        assert_equal 0, Sidekiq2::Queue.new("queue_5").size
        assert_equal 1, Sidekiq2::Queue.new("queue_6").size
      ensure
        Sidekiq2.client_middleware.remove Stopper
      end
    end
      ...
      Time.stub(:now, enqueued_time) do
        ...
        Sidekiq2.redis do |conn|
          %w(queue:queue_1 queue:queue_2 queue:queue_4 queue:queue_5).each do |queue_name|
            ...
            job = Sidekiq2.load_json(conn.lrange(queue_name, 0, -1)[0])
    ...
    def with_sidekiq_option(name, value)
      _original, Sidekiq2.options[name] = Sidekiq2.options[name], value
      begin
        yield
      ensure
        Sidekiq2.options[name] = _original
      end
    end
