...
- Update Sidekiq2::Client patches to work with new Module#prepend
...
- Refactor batch job success/failure to gracefully handle several edge
  cases with regard to Sidekiq2::Shutdown.  This should greatly reduce
...
```ruby
Sidekiq2::Pro.dogstatsd = ->{ Datadog::Statsd.new("metrics.example.com", 8125) }
```
...
```ruby
Sidekiq2::Pro.statsd = ->{ ::Statsd.new("127.0.0.1", 8125) }
```
...
- Re-implement `Sidekiq2::Queue#delete_job` to avoid O(n) runtime [#3408]
...
- Reimplement Sidekiq2::JobSet#find\_job with ZSCAN [#3197]
...
- Don't delete batches immediately upon success but set a 24 hr expiry, this allows
  Sidekiq2::Batch::Status#poll to work, even after batch success. [#3011]
- New `Sidekiq2::PendingSet#destroy(jid)` API to remove poison pill jobs [#3015]
...
- Fixed detection of missing batches, `NoSuchBatch` should be raised
  properly now if `Sidekiq2::Batch.new(bid)` is called on a batch no
  longer in Redis.
...
- Convert Lua-based `Sidekiq2::Queue#delete_by_class` to Ruby-based, to
...
- Officially deprecate Sidekiq2::Notifications code.  Notifications have
...
- Ignore redundant calls to `Sidekiq2::Client.reliable_push!` [#2408]
...
mount Sidekiq2::Pro::Web => '/sidekiq' # default
mount Sidekiq2::Pro::Web.with(redis_pool: POOL1), at: '/sidekiq2', as: 'sidekiq2' # shard1
mount Sidekiq2::Pro::Web.with(redis_pool: POOL2), at: '/sidekiq2', as: 'sidekiq2' # shard2
```
...
- SCAN support in the Sidekiq2::SortedSet API.  Here's an example that
  ...
  **Redis 2.8**
```ruby
  Sidekiq2::RetrySet.new.scan("Warehouse::OrderShip") do |job|
    job.delete
  end
```

- Sidekiq2::Batch#jobs now returns the set of JIDs added to the batch.
- Sidekiq2::Batch#jids returns the complete set of JIDs associated with the batch.
- Sidekiq2::Batch#remove\_jobs(jid, jid, ...) removes JIDs from the set, allowing early termination of jobs if they become irrelevant according to application logic.
- Sidekiq2::Batch#include?(jid) allows jobs to check if they are still
  relevant to a Batch and exit early if not.
- Sidekiq2::SortedSet#find\_job(jid) now uses server-side Lua if possible **Redis 2.6** [jonhyman]
...
- Update reliable\_push to work with Sidekiq2::Client refactoring in 2.16
...
- Add Sidekiq2::Batch::Status#delete [#1205]
...
- Add fast Sidekiq2::Queue#delete\_job(jid) API which leverages Lua so job lookup is
...
- Add fast Sidekiq2::Queue#delete\_by\_class(klass) API to remove all
  jobs of a given type.  Uses server-side Lua for performance. **Redis 2.6**

1.1.0
-----------

- New `sidekiq/pro/reliable_push` which makes Sidekiq2::Client resiliant
...
```ruby
def perform(...)
  batch = Sidekiq2::Batch.new(bid) # instantiate batch associated with this job
  batch.jobs do
