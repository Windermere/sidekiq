...
* API changes:
  - `Sidekiq1::Client.registered_workers` replaced by `Sidekiq1::Workers.new`
  - `Sidekiq1::Client.registered_queues` replaced by `Sidekiq1::Queue.all`
  - `Sidekiq1::Worker#retries_exhausted` replaced by `Sidekiq1::Worker.sidekiq_retries_exhausted`
  - `Sidekiq1::Workers#each` has changed significantly with a reworking
