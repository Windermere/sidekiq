...
```ruby
limiter = Sidekiq2::Limiter.unlimited(...any args...)
```
...
```ruby
expiry = 1.month.to_i; Sidekiq2::Limiter.redis { |c| c.scan_each(match: "lmtr-cfree-*") { |key| c.expire(key, expiry) } }
```
...
- Rework `Sidekiq2::Limiter` redis handling to match global redis handling.
